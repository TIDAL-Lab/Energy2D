...
class Model2D {
  ...
  static const double AIR_THERMAL_CONDUCTIVITY = 0.025;
  static const int AIR_SPECIFIC_HEAT = 1012;
  static const double AIR_DENSITY = 1.204;
  static const double AIR_VISCOSITY = 0.00001568;
  bool valid = false;
  int indexOfStep = 0;
  ...
  Model2D(this.nx, this.ny) {
    ...
    // inside temperature initialization
    for (int i=40; i>=18; i--) {
       for (int j=18; j<=i; j++) {
          t[i][40-j] = 10.0;
    ...
    for(int i=20;i<=59;i++) {
      for(int j=21;j<=ny-4;j++) {
        t[i][j]=10.0;
      }
    }
    //lower vertical walls
       for (int j=55; j<ny-2; j++) {
         fluidity[19][j] = false;
         conductivity[19][j] = 0.12;

         fluidity[20][j] = false;
         conductivity[20][j] = 0.12;

         fluidity[59][j] = false;
         conductivity[59][j] = 0.12;

         fluidity[60][j] = false;
         conductivity[60][j] = 0.12;
       }

     //window and corresponding wall
       for (int j=32; j<55; j++) {
        fluidity[20][j] = false;
        conductivity[20][j] = 1.0;

        fluidity[19][j] = false;
        conductivity[19][j] = 1.0;
      ...
      //vertical wall above window and corresponding wall
       for (int j=21; j<32; j++) {
         fluidity[19][j] = false;
         conductivity[19][j] = 0.12;
     ...
     // horizontal base of house/ ground
       for (int i=3; i<nx-3; i++) {

         fluidity[i][ny-3] = false;
         conductivity[i][ny-3] = 0.12;

         fluidity[i][ny-2] = false;
         conductivity[i][ny-2] = 0.12;

         fluidity[i][ny-4] = false;
         conductivity[i][ny-4] = 0.12;
        }
    ...
    //triple lined roof
         for (int j=0; j<=25; j++) {
           ...
           conductivity[39-j][j] = 0.12;
           specificHeat[39-j][j] = 840.0;

           fluidity[40-j][j] = false;
           conductivity[40-j][j] = 0.12;
 ...
 //  horizontal wall just below sloping roof
       for (int i=19; i<60; i++) {
        fluidity[i][21] = false;
        conductivity[i][21] = 0.15;
        fluidity[i][22] = false;
        conductivity[i][22] = 0.15;
