  ...
  Model2D(this.nx, this.ny) {
   ...
   tb = new Matrix<double> (nx,ny);
    ...
    /*   for (int i=0; i<3; i++) {
     ...
     //   fluidity[nx~/2][ny~/2] = false;
     //   fluidity[nx~/2][ny-4] = false;

        conductivity[nx~/2-i][j] = 0.0;
        conductivity[nx~/2-1+i][j] = 0.0;
       }
    }

    for (int i= -8; i< 8 ; i++) {
      for (int j = 1; j< 3; j++) {
            fluidity[nx ~/2 + i][ny - 3 - j] = false;
      }
    }
 */
       ...
       for (int i= 0; i<1; i++) {
         for (int j = ny-1-30; j < ny-1-10 ; j++) {

          fluidity[nx-1-i][j] = false;
          conductivity[nx-1-i][j] = 0.98;
        }
      }
