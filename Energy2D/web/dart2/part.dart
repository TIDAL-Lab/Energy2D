 ...
 */
 ...
 abstract class Manipulable {
  ...
  Manipulable(Shape shape) {
    setShape(shape);
  }

  Manipulable duplicate(double x, double y);
   ...
   * Point2D.Float getCenter() {
    ...
    return new Point2D.Float((double) shape.getBounds2D().getCenterX(), (double) shape.getBounds2D().getCenterY());
  }
  */
 ...
 class Part extends Manipulable {

   Part() : super(0.0);

  // Stefan's constant unit J/(s*m^2*K^-4)
    static double STEFAN_CONSTANT = 0.0000000567;
  ...
  */
  double unitSurfaceArea = 100.0;

  static int polygonize = 50;
  static double radiatorSpacing = .5;
  static double MINIMUM_RADIATING_TEMPERATUE = 20.0;
  static double SIN30 = 0.5;
  static double COS30 = 0.866;
  static double SIN60 = 0.866;
  static double COS60 = 0.5;

  /*
   * static DecimalFormat LABEL_FORMAT = new DecimalFormat("####.######");
...
*/
   void setConstantTemperature(bool b) {
    constantTemperature = b;
  }

   bool getConstantTemperature() {
  ...
  String getLabel(String label, Model2D model, bool useFahrenheit) {
    if (label == null)
      return null;
    if (label.indexOf('%') == -1)
      return label;
